...
const showTable = async (req, res) => {
  ...
  // делаем запрос на данные Buh
  const urlBuh = `https://focus-api.kontur.ru/api3/buh?inn=${inn}&ogrn=${ogrn}&key=${KEY}`;
  // делаем запрос на данные Req
  const urlReq = `https://focus-api.kontur.ru/api3/req?inn=${inn}&ogrn=${ogrn}&key=${KEY}`;
  // делаем запрос на данные Analytics
  const urlAnalytics = `https://focus-api.kontur.ru/api3/analytics?inn=${inn}&ogrn=${ogrn}&key=${KEY}`;
  ...
  async function algorythmBuh() {
    const response = await fetch(urlBuh);
  ...
  async function algorythmAnalytics() {
    try {
      let response = await fetch(urlAnalytics);
      let data = await response.json();
